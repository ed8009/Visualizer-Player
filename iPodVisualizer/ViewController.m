...
@property (strong, nonatomic) UIBarButtonItem *effect;
...
- (void)configureBars {
    ...
    self.effect = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemRefresh target:self action:@selector(updateEffect)];
    ...
    self.playItems = [NSArray arrayWithObjects:pickBBI, leftFlexBBI, _playBBI, rightFlexBBI, _effect, nil];
    self.pauseItems = [NSArray arrayWithObjects:pickBBI, leftFlexBBI, _pauseBBI, rightFlexBBI, _effect, nil];
...
- (void)configureAudioSession {
    ...
    if (error){
        NSLog(@"Error seting category: %@", [error description]);
    }
}

- (void)updateEffect {
    self.effectEnable = !self.effectEnable;
    self.visualizer.effectEnable = self.effectEnable;
}
